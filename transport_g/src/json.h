...
namespace Json {
  ...
  public:
    ...
    const Svg::Color& AsColor() const {
    	if (std::holds_alternative<std::string>(*this)) {
    		return std::get<std::string>(*this);
    	} else if (std::get<std::vector<Node>>(*this).size() == 3) {
    		auto& v = std::get<std::vector<Node>>(*this);
    		Svg::Rgb rgb;
    		rgb.red = v[0].AsInt();
    		rgb.green = v[1].AsInt();
    		rgb.blue = v[2].AsInt();
    		return rgb;
    	} else if (std::get<std::vector<Node>>(*this).size() == 4) {
    		auto& v = std::get<std::vector<Node>>(*this);
    		Svg::Rgba rgba;
    		rgba.red = v[0].AsInt();
    		rgba.green = v[1].AsInt();
    		rgba.blue = v[2].AsInt();
    		rgba.alpha = v[3].AsDouble();
    		return rgba;
    	}
    }
