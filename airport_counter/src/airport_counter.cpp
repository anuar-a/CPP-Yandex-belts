...
public:
  ...
  void Insert(TAirport airport) {
	  items[static_cast<size_t>(airport)] += 1;
  }
  ...
  Items GetItems() const {
	  ...
	  for (int i = 0; i < items.size(); i++) {
		  m[i] = make_pair(static_cast<TAirport>(i),items[i]);
	  }
	  return m;
  }

private:
  array<size_t, static_cast<size_t>(TAirport::Last_)> items;
};
