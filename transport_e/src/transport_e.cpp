...
struct Bus {
	...
	bool is_roundtrip;
	Bus() = default;
	Bus(vector<string> stops_, bool is_roundtrip_):stops(stops_), is_roundtrip(is_roundtrip_){}
};

class TransportManager {
public:
	unordered_map<string, Bus> GetBuses(){
		return buses_data;
	}
	...
	size_t GetBusesSize(){
		return buses_data.size();
	}

	size_t GetStopsSize(){
		return stops_data.size();
	}
	...
	void AddBus(string& name, vector<string>& stops, bool is_roundtrip) {
		buses_data.insert({name, {stops, is_roundtrip}});
		for (auto& stop : stops) {
...
void IOProcessJson (istream& is, ostream& os){
	...
	os.precision(6);
	TransportManager tm;
	unordered_map<string, Graph::VertexId> name_to_id;
	auto doc = Json::Load(is);
	auto& root = doc.GetRoot();
	auto& m = root.AsMap();
	auto& base_requests = m.at("base_requests").AsArray();
	auto& stat_requests = m.at("stat_requests").AsArray();
	auto& routing_settings = m.at("routing_settings").AsMap();
	auto bus_wait_time = routing_settings.at("bus_wait_time").AsNumber();
	auto bus_velocity = routing_settings.at("bus_velocity").AsNumber();
	...
	for (auto& request:base_requests) {
		...
		if (req_map.at("type").AsString() == "Stop") {
			...
			tm.AddBus(bus_name, stops_string, is_roundtrip);
		}
	}
	auto num_of_stops = tm.GetStopsSize();
	auto stops_data = tm.GetStops();
	auto buses_data = tm.GetBuses();
	for (auto& [stop, data] : stops_data) {
		name_to_id[stop] = name_to_id.size();
	}
	for (auto& [stop, data] : stops_data) {
		name_to_id[stop + stop] = name_to_id.size();
	}
	Graph::DirectedWeightedGraph<double> graph(num_of_stops * 2);
	for (auto& [name, bus] : tm.GetBuses()) {
		if (bus.is_roundtrip) {
			for (size_t i = 0; i < bus.stops.size(); ++i) {
				auto u = bus.stops[i];
				graph.AddEdge({name_to_id[u],
							  name_to_id[u + u],
							  bus_wait_time,
							  "Wait",
							  u,
					...
					for (size_t j = i + 1; j < bus.stops.size(); ++j) {
						...
						while (n <= j) {
							auto u = bus.stops[m];
							auto v = bus.stops[n];
							bus_time += (stops_data[u].distances[v] * 60) / (bus_velocity * 1000);
							m++;
							n++;
						}
						graph.AddEdge({name_to_id[u + u],
									  name_to_id[v],
									  bus_time,
									  "Bus",
									  name,
									  j - i});
						//cout << "Edge " << u + u << " " << v << " " << bus_time << " " << j - i << endl;
					}
			...
			for (size_t i = mid; i < bus.stops.size() - 1; ++i) {
				auto u = bus.stops[i];
				graph.AddEdge({name_to_id[u],
							  name_to_id[u + u],
							  bus_wait_time,
	...
	for (auto& request:stat_requests) {
		...
		if (req_map.at("type").AsString() == "Stop") {
			...
			stringstream os;
			auto from = req_map.at("from").AsString();
			auto to = req_map.at("to").AsString();
			auto ans = router.BuildRoute(name_to_id[from], name_to_id[to]);
			os << '{';
			if (ans.has_value()) {
				os << "\"items\": [";
				auto route_id = ans.value().id;
				auto edge_count = ans.value().edge_count;
				//cout << route_id << " " << ans.value().weight << " " << edge_count << endl;
				int cnt = 1;
				for (int edge_idx = 0; edge_idx < edge_count; ++edge_idx) {
					auto edge_id = router.GetRouteEdge(route_id, edge_idx);
					auto edge = graph.GetEdge(edge_id);
					os << '{';
					os << "\"time\"" << ": " << edge.weight << ',';
					if (edge.type == "Bus") {
						os << "\"bus\"" << ": " << '"' << edge.route_no << '"' << ',';
						os << "\"span_count\"" << ": " << edge.span_count << ',';
					} else if (edge.type == "Wait") {
						os << "\"stop_name\"" << ": " << '"' << edge.route_no << '"' << ',';
					}
					os << "\"type\"" << ": " << '"' << edge.type << '"';
					os << '}';
					//cout //<< edge.from << " "
				...
				os << "],";
				os << "\"request_id\"" << ": " << id << ',';
				os << "\"total_time\"" << ": " << ans.value().weight;
			} else {
