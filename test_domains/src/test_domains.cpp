...
void TestSplit() {
	ASSERT_EQUAL(Split("example.com", "."), vector<string_view>({"example", "com"}));
}

void TestGetReversedParts() {
	Domain domain("example.com.ru");
	const auto test_range = domain.GetReversedParts();
	vector<string_view> test(begin(test_range), end(test_range));
	ASSERT_EQUAL(test, vector<string_view>({"ru", "com", "example"}));
}

void TestSubdomain() {
	Domain d1("example.com");
	ASSERT_EQUAL(IsSubdomain(d1, d1), true);
}

void TestIsSubdomainArgOrder() {
	Domain d1("example.com");
	Domain d2("ru.example.com");
	ASSERT_EQUAL(IsSubdomain(d2, d1), true);
}
...
void TestDomainChecker() {
	...
	ASSERT_EQUAL(
			DomainChecker(begin(domains), end(domains)).IsSubdomain(Domain("ya.example.com")),
			true
	);
}
...
int main() {
  ...
  RUN_TEST(tr, TestIsSubdomainArgOrder);
