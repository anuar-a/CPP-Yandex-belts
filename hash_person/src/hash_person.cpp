...
struct AddressHasher {
	size_t operator() (const Address& a) const{
	  size_t r1 = hash_city(a.city);
	  size_t r2 = hash_street(a.street);
	  size_t r3 = hash_building(a.building);
	  // ax^2 + bx + c
	  size_t x = 2946901;
...
struct PersonHasher {
	size_t operator() (const Person& p) const{
	  size_t r1 = hash_name(p.name);
	  size_t r2 = hash_height(p.height);
	  size_t r3 = hash_weight(p.weight);
	  size_t r4 = hash_address(p.address);
	  // ax^2 + bx + c
	  size_t x = 2946901;
