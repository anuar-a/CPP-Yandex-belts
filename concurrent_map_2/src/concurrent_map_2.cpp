...
public:
  ...
  explicit ConcurrentMap(size_t bucket_count) : bucket_count_(bucket_count), maps(bucket_count) {

  }

  WriteAccess operator[](const K& key) {
	  return {lock_guard(maps[Indexer(key)].m), maps[Indexer(key)].data[key]};
  }
  ReadAccess At(const K& key) const {
	  if (Has(key)) {
		  return {lock_guard(maps.at(Indexer(key)).m), maps.at(Indexer(key)).data.at(key)};
	  } else {
  ...
  bool Has(const K& key) const {
	  return maps.at(Indexer(key)).data.count(key) > 0;
  }
  ...
  MapType BuildOrdinaryMap() const {
	  ...
	  for (auto& item : maps) {
		  ...
		  result.insert(item.data.begin(), item.data.end());
	  }
	  return result;
  }

  size_t Indexer(const K& key) const {
